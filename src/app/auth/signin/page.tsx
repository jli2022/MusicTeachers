...
function SignInForm() {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (result?.error) {
        setError('Google sign-in is not properly configured. Please contact an administrator.')
      } else if (result?.url) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {/* Google Sign In for Teachers - Only show if Google OAuth is configured */}
        {showGoogleOAuth && (
          <div className="mt-8">
            ...
            {showGoogleOAuth && (
              <div>
                <span className="text-gray-600 text-sm">
                  Teachers can sign up with Google above
                </span>
              </div>
            )}
            <div>
              <span className="text-gray-600 text-sm">
                Employers: Contact an administrator for account access
              </span>
            </div>
          </div>
        </form>
